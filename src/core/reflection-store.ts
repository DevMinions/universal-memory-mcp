...
export function buildReflectionStorePayloads(params: BuildReflectionStorePayloadsParams): {
  ...
  const eventPayload = buildReflectionEventPayload({
    eventId,
    scope: params.scope,
    sessionKey: params.sessionKey,
    sessionId: params.sessionId,
  ...
  const payloads: ReflectionStorePayload[] = [
    { ...eventPayload, metadata: eventPayload.metadata as unknown as Record<string, unknown> },
  ];
  ...
  payloads.push(...itemPayloads.map((p) => ({ ...p, metadata: p.metadata as unknown as Record<string, unknown> })));
